...
async function getTraditionalVsMaximaIb(
  locale: string
): Promise<TraditionalVsMaximaIBData> {
  try {
    const lang =
    ...
    const data: RawTraditionalVsMaximaIBData = Array.isArray(result.data)
      ? result.data[0]
      : result.data;
    ...
    return {
      ...
      subtitle:
        source.subtitle ||
        "Discover how Maxima IB 2.0 solves the inherent problems of traditional introducing broker models",
      traditional_title: source.traditional_title || "TRADITIONAL I.B",
      traditional_subtitle:
        source.traditional_subtitle ||
      ...
      maxima_subtitle:
        source.maxima_subtitle || "Revolutionizing Introducing Broker Concept",
      ...
      step_description_1:
        source.step_description_1 ||
        "Unlike traditional models that require constant client acquisition, Maxima IB 2.0 builds sustainable, long-term business relationships.",
      step_title_2: source.step_title_2 || "Revenue Model",
      step_description_2:
        source.step_description_2 ||
        "Maxima's revenue-sharing model provides consistent income for IBs, even during market fluctuations, unlike traditional volume-dependent models.",
      step_title_3: source.step_title_3 || "Client Retention",
      step_description_3:
        source.step_description_3 ||
        "With Maxima's AI-driven trading system, clients achieve consistent profitability, resulting in dramatically improved client retention rates.",
    };
  } catch (error) {
...
export default function MaximaVsTraditionalSection() {
  ...
  const [activeTab, setActiveTab] = useState<"traditional" | "maxima">(
    "maxima"
  );
    ...
    <ConfigProvider theme={themeConfig}>
      <div
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-4 relative z-20">
          ...
          <div className="md:hidden mb-8 flex space-x-2 justify-center px-2">
            <button
              onClick={() => setActiveTab("traditional")}
              className="px-1 rounded-lg cursor-pointer font-medium transition-all">
              <span
                className={`block px-3 py-2 text-sm sm:text-base cursor-pointer rounded-lg font-medium transition-all ${
                  activeTab === "traditional"
                    ? mytheme === "light"
                      ? "bg-red-500 text-white shadow-lg shadow-green-200"
                      : "bg-red-700 text-white shadow-lg shadow-green-900/30"
                    : mytheme === "light"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-16 items-stretch">
            <motion.div
              ...
              <div
                ...
                <motion.p className="text-white text-lg" variants={fadeInUp}>
                  ...
                  traditional_points.map((point, index) => (
                    <motion.div
                      ...
                      className={`p-4 rounded-lg ${
                        mytheme === "light" ? "bg-red-50" : "bg-red-900/20"
                      }`}
              ...
              <div
                ...
                <p
                  className={`text-sm ${
                    mytheme === "light" ? "text-red-600" : "text-red-300"
                  }`}>
                  <span className="material-symbols-outlined text-sm align-middle mr-1">
                    warning
                  </span>
                  Traditional IB models lead to unstable income and high client
                  turnover
                </p>
              </div>
            </motion.div>
            ...
            <motion.div
                ...
                {maxima_points.length > 0 ? (
                  maxima_points.map((point, index) => (
                    <motion.div
                      key={index}
                      className={`p-4 rounded-lg ${
                        mytheme === "light" ? "bg-green-50" : "bg-green-900/20"
                      }`}
                      variants={pointVariants}>
                      <div className="flex items-center gap-3 mb-2">
                        <div
              ...
              <div
                ...
                <p
                  className={`text-sm ${
                    mytheme === "light" ? "text-green-600" : "text-green-300"
                  }`}>
                  <span className="material-symbols-outlined text-sm align-middle mr-1">
                    verified
                  </span>
                  Maxima IB 2.0 ensures sustainable income and long-term client
                  relationships
                </p>
              </div>
            </motion.div>
