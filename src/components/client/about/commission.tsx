...
async function getCommission(
  locale: string
): Promise<TransformedCommissionData> {
  try {
    const lang =
    ...
    const data: RawCommissionData = Array.isArray(result.data)
      ? result.data[0]
      : result.data;
    ...
    const profits_sharing_3 =
      lang === "en-US" ? data.profits_sharing_3 : source.profits_sharing_3;
...
export default function CommissionSection({ id }: CommissionProps) {
  ...
  return (
    <ConfigProvider theme={themeConfig}>
      <section
        id={id}
        className={`py-12 md:py-20 relative overflow-hidden font-inter ${
